...
--
-- `note` is for what is not a number at all: traction control counts 1 as
-- MOST intervention and 11 as least, which no multiplier can express and
-- which was re-derived wrongly more than once.
...
-- Facts about an entry that are not a number: which direction the scale
-- runs, what the units really are. One row per entry, replaced wholesale.
...
def _migrate(conn) -> list[str]:
    ...
    # v12 adds display_observations and display_notes, both new tables --
...
def backfill_excursions(conn, lap_ids: list[int] | None = None) -> int:
    ...
    q = ("SELECT id, invalid, max_tyres_out, excursions, off_track_ms"
         " FROM laps WHERE invalid_source = 'inferred' AND sample_stride = 1")
    args: list = []
