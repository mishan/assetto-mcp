...
| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `tobi-ac-tracer-csp-lua`                                |
| Version    | 1.0.2                                     |
| Author     | tobi                                 |
| Category   | gaming                                    |
| Installs   | 13                                |
...
**GitHub:** [tobi/ac-tracer](https://github.com/tobi/ac-tracer) — ⭐ 7 | Forks: 2
...
This CSP Lua Skill is a focused API reference for Assetto Corsa modders working with the Custom Shaders Patch environment. It documents the ac.*, ui.*, render.*, physics.* namespaces, functions, parameter lists, return types, and event hooks used to read vehicle state, draw custom HUDs, create telemetry overlays, register input handlers, and integrate custom shaders. Use it when writing or debugging CSP Lua scripts to access game data (speed, RPM, tire info, collisions), render 2D/3D overlays, manage lifecycle callbacks (init, update, draw), and manipulate physics-related properties safely. The reference includes examples, best practices for performance and resource management, compatibility notes across CSP versions, and debugging tips to accelerate development, reduce runtime errors, and ensure stable, interoperable mods for both novice and experienced creators.
...
```bash
npx -y @lobehub/market-cli skills install tobi-ac-tracer-csp-lua
```
...
npx -y @lobehub/market-cli skills install tobi-ac-tracer-csp-lua --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install tobi-ac-tracer-csp-lua --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install tobi-ac-tracer-csp-lua --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install tobi-ac-tracer-csp-lua --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install tobi-ac-tracer-csp-lua --version 1.0.2
```
...
```bash
npx -y @lobehub/market-cli skills rate tobi-ac-tracer-csp-lua --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment tobi-ac-tracer-csp-lua -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment tobi-ac-tracer-csp-lua
```
...
```bash
npx -y @lobehub/market-cli skills comments tobi-ac-tracer-csp-lua
```
